...
CREATE TABLE IF NOT EXISTS student_communication_threads (
  ...
  parent_id varchar REFERENCES users(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS student_communication_messages (
  id varchar PRIMARY KEY DEFAULT gen_random_uuid()::text,
  thread_id varchar NOT NULL REFERENCES student_communication_threads(id) ON DELETE CASCADE,
  student_id varchar NOT NULL REFERENCES students(id) ON DELETE CASCADE,
  tutor_id varchar NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  parent_id varchar REFERENCES users(id) ON DELETE SET NULL,
  audience communication_audience NOT NULL,
  ...
  sender_student_user_id varchar REFERENCES student_users(id) ON DELETE SET NULL,
