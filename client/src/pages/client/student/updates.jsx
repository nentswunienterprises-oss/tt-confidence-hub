...
export default function StudentUpdates() {
    ...
    var _a = useQuery({
        ...
        retry: false,
    }), broadcasts = _a.data, isLoading = _a.isLoading;
    return (<div className="space-y-4 sm:space-y-6">
      ...
      {isLoading ? (<div className="flex items-center justify-center h-48 sm:h-64">
          <div className="w-10 h-10 sm:w-12 sm:h-12 border-4 border-primary/20 border-t-primary rounded-full animate-spin"/>
        </div>) : broadcasts && broadcasts.length > 0 ? (<div className="space-y-3 sm:space-y-4">
          {broadcasts.map(function (broadcast) { return (<Card key={broadcast.id}>
