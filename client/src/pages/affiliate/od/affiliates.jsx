...
export default function ODAffiliates() {
    ...
    }), _d = _c.data, affiliates = _d === void 0 ? [] : _d, dataLoading = _c.isLoading, dataError = _c.error;
    useEffect(function () {
    ...
    var filteredAffiliates = affiliates.filter(function (affiliate) {
    ...
    return (<DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="relative">
          ...
          <Input placeholder="Search affiliates..." value={searchQuery} onChange={function (e) { return setSearchQuery(e.target.value); }} className="pl-10"/>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <Card className="p-4">
            <p className="text-2xl font-bold text-green-600">
              {affiliates.filter(function (a) { return a.status === "active"; }).length}
            </p>
            <p className="text-sm text-muted-foreground">Active</p>
          </Card>
          <Card className="p-4">
            <p className="text-2xl font-bold text-primary">
              {affiliates.reduce(function (sum, a) { return sum + a.totalEncounters; }, 0)}
            </p>
            <p className="text-sm text-muted-foreground">Total Encounters</p>
          </Card>
          <Card className="p-4">
            <p className="text-2xl font-bold text-green-600">
              {affiliates.reduce(function (sum, a) { return sum + a.totalCloses; }, 0)}
            </p>
        ...
        {/* Affiliates Grid */}
        {filteredAffiliates.length > 0 ? (<div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredAffiliates.map(function (affiliate) { return (<Card key={affiliate.id} className="p-4">
