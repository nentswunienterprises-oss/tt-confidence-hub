...
export default function ODEncounters() {
  ...
  const { data: encounters = [], isLoading: dataLoading, error: dataError } = useQuery<Encounter[]>({
  ...
  const filteredEncounters = encounters.filter((encounter) =>
    encounter.parentName.toLowerCase().includes(searchQuery.toLowerCase()) ||
    encounter.parentEmail?.toLowerCase().includes(searchQuery.toLowerCase())
  );
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <Card className="p-4">
            <p className="text-2xl font-bold text-blue-600">
              {encounters.filter((e) => e.status === "new").length}
            </p>
            <p className="text-sm text-muted-foreground">New</p>
          </Card>
          <Card className="p-4">
            <p className="text-2xl font-bold text-yellow-600">
              {encounters.filter((e) => e.status === "contacted").length}
            </p>
            <p className="text-sm text-muted-foreground">Contacted</p>
          </Card>
          <Card className="p-4">
            <p className="text-2xl font-bold text-green-600">
              {encounters.filter((e) => e.status === "converted").length}
            </p>
