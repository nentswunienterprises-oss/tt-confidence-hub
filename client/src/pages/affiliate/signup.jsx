...
export default function AffiliateSignup() {
    var _a = useState(null), selectedRole = _a[0], setSelectedRole = _a[1];
    var _b = useState("signup"), mode = _b[0], setMode = _b[1];
    var navigate = useNavigate();
    if (!selectedRole) {
        return (<div className="min-h-screen flex flex-col" style={{ backgroundColor: "#FFF5ED" }}>
        {/* Header */}
        <header className="fixed top-0 left-0 right-0 z-50 backdrop-blur-md" style={{ backgroundColor: "rgba(255, 245, 237, 0.95)" }}>
          <div className="max-w-7xl mx-auto px-3 sm:px-6 md:px-12 h-16 sm:h-20 flex items-center justify-between">
            <Button variant="ghost" className="text-sm sm:text-base font-medium hover:bg-transparent flex items-center gap-1 sm:gap-2 px-2 sm:px-4" style={{ color: "#1A1A1A" }} onClick={function () { return navigate("/affiliate/landing"); }}>
        ...
        <div className="flex-1 flex items-center justify-center px-4 sm:px-6 py-8 sm:py-12">
          <div className="w-full max-w-3xl space-y-6 sm:space-y-10">
            ...
            <div className="grid md:grid-cols-2 gap-4 sm:gap-8">
              {/* Affiliate Option */}
              <Card className="p-4 sm:p-8 border-0 shadow-lg hover:shadow-xl transition-all cursor-pointer group" style={{ backgroundColor: "white" }} onClick={function () { return setSelectedRole("affiliate"); }}>
                ...
                <ul className="space-y-1.5 sm:space-y-2 mb-4 sm:mb-6">
                  {["Personal QR code", "Real-time tracking", "Easy payouts"].map(function (item, i) { return (<li key={i} className="flex items-center gap-2 text-xs sm:text-sm" style={{ color: "#5A5A5A" }}>
                      <Check className="w-3 h-3 sm:w-4 sm:h-4" style={{ color: "#E63946" }}/>
                      {item}
                    </li>); })}
                </ul>
                <Button className="w-full rounded-full py-4 sm:py-6 font-semibold border-0 text-sm sm:text-base" style={{ backgroundColor: "#E63946", color: "white" }}>
                  Become an Affiliate
              ...
              <Card className="p-4 sm:p-8 border-0 shadow-lg hover:shadow-xl transition-all cursor-pointer group relative" style={{ backgroundColor: "white" }} onClick={function () { return setSelectedRole("od"); }}>
                <div className="absolute top-3 right-3 sm:top-4 sm:right-4 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-semibold" style={{ backgroundColor: "#E63946", color: "white" }}>
                  Popular
                </div>
                <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform" style={{ backgroundColor: "#FFF0F0" }}>
                ...
                <ul className="space-y-1.5 sm:space-y-2 mb-4 sm:mb-6">
                  {["Multiple campaigns", "School partnerships", "Bonus tiers"].map(function (item, i) { return (<li key={i} className="flex items-center gap-2 text-xs sm:text-sm" style={{ color: "#5A5A5A" }}>
                      <Check className="w-3 h-3 sm:w-4 sm:h-4" style={{ color: "#E63946" }}/>
                      {item}
                    </li>); })}
                </ul>
    ...
    return (<div className="min-h-screen flex flex-col" style={{ backgroundColor: "#FFF5ED" }}>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 backdrop-blur-md" style={{ backgroundColor: "rgba(255, 245, 237, 0.95)" }}>
        <div className="max-w-7xl mx-auto px-3 sm:px-6 md:px-12 h-16 sm:h-20 flex items-center justify-between">
          <Button variant="ghost" className="text-sm sm:text-base font-medium hover:bg-transparent flex items-center gap-1 sm:gap-2 px-2 sm:px-4" style={{ color: "#1A1A1A" }} onClick={function () { return setSelectedRole(null); }}>
            <ArrowLeft className="w-3.5 h-3.5 sm:w-4 sm:h-4"/>
            Back
          </Button>
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-6 py-8 sm:py-12">
        <div className="w-full max-w-md space-y-6 sm:space-y-8">
          {/* Hero Section */}
          <div className="text-center space-y-3 sm:space-y-4">
            <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center mx-auto" style={{ backgroundColor: "#FFF0F0" }}>
              {selectedRole === "affiliate" ? (<QrCode className="w-7 h-7 sm:w-8 sm:h-8" style={{ color: "#E63946" }}/>) : (<Compass className="w-7 h-7 sm:w-8 sm:h-8" style={{ color: "#E63946" }}/>)}
            </div>
          ...
          <div className="flex gap-2 justify-center p-1 rounded-full" style={{ backgroundColor: "#FFF0F0" }}>
            <button className={"flex-1 py-3 px-6 rounded-full font-semibold transition-all ".concat(mode === "signup" ? "shadow-md" : "")} style={{
            backgroundColor: mode === "signup" ? "#E63946" : "transparent",
            color: mode === "signup" ? "white" : "#5A5A5A"
        }} onClick={function () { return setMode("signup"); }}>
              Sign Up
            </button>
            <button className={"flex-1 py-3 px-6 rounded-full font-semibold transition-all ".concat(mode === "login" ? "shadow-md" : "")} style={{
            backgroundColor: mode === "login" ? "#E63946" : "transparent",
            color: mode === "login" ? "white" : "#5A5A5A"
        }} onClick={function () { return setMode("login"); }}>
              Login
            </button>
          </div>
