...
export default function ExecutiveSignup() {
    ...
    var _b = useState("login"), mode = _b[0], setMode = _b[1];
    ...
    if (!selectedRole) {
        ...
        <header className="fixed top-0 left-0 right-0 z-50 backdrop-blur-md" style={{ backgroundColor: "rgba(255, 245, 237, 0.95)" }}>
          <div className="max-w-7xl mx-auto px-3 sm:px-6 md:px-12 h-16 sm:h-20 flex items-center justify-between">
            <Button variant="ghost" className="text-sm sm:text-base font-medium hover:bg-transparent flex items-center gap-1 sm:gap-2 px-2 sm:px-4" style={{ color: "#1A1A1A" }} onClick={function () { return navigate("/executive/landing"); }}>
        ...
        <div className="flex-1 flex items-center justify-center px-4 sm:px-6 py-6 sm:py-12">
          <div className="w-full max-w-5xl space-y-6 sm:space-y-10">
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
              {Object.keys(roleData).map(function (roleKey) {
                var role = roleData[roleKey];
                var Icon = role.icon;
                return (<Card key={roleKey} className="p-4 sm:p-6 border-0 shadow-lg hover:shadow-xl transition-all cursor-pointer group" style={{ backgroundColor: "white" }} onClick={function () { return setSelectedRole(roleKey); }}>
                    <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center mb-3 sm:mb-5 group-hover:scale-110 transition-transform" style={{ backgroundColor: "#FFF0F0" }}>
                    ...
                    <Button className="w-full rounded-full py-3 sm:py-5 font-semibold text-sm sm:text-base" style={{ backgroundColor: "#E63946", color: "white" }}>
    ...
    var currentRole = roleData[selectedRole];
    var Icon = currentRole.icon;
    return (<div className="min-h-screen flex flex-col" style={{ backgroundColor: "#FFF5ED" }}>
      {/* Header */}
      <header className="fixed top-0 left-0 right-0 z-50 backdrop-blur-md" style={{ backgroundColor: "rgba(255, 245, 237, 0.95)" }}>
        <div className="max-w-7xl mx-auto px-3 sm:px-6 md:px-12 h-16 sm:h-20 flex items-center justify-between">
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-6 py-6 sm:py-12">
        <div className="w-full max-w-md space-y-5 sm:space-y-8">
          {/* Hero Section */}
          <div className="text-center space-y-3 sm:space-y-4">
            <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center mx-auto" style={{ backgroundColor: "#FFF0F0" }}>
          ...
          <div className="flex gap-2 justify-center p-1 rounded-full" style={{ backgroundColor: "#FFF0F0" }}>
            <button className={"flex-1 py-2 sm:py-3 px-4 sm:px-6 rounded-full font-semibold text-sm sm:text-base transition-all ".concat(mode === "login" ? "shadow-md" : "")} style={{
            backgroundColor: mode === "login" ? "#E63946" : "transparent",
            color: mode === "login" ? "white" : "#5A5A5A"
        }} onClick={function () { return setMode("login"); }}>
              Login
            </button>
            <button className={"flex-1 py-2 sm:py-3 px-4 sm:px-6 rounded-full font-semibold text-sm sm:text-base transition-all ".concat(mode === "signup" ? "shadow-md" : "")} style={{
          ...
          <Card className="p-4 sm:p-8 border-0 shadow-lg" style={{ backgroundColor: "white" }}>
            <ExecutiveAuthForm role={selectedRole} mode={mode} setMode={setMode}/>
          </Card>
